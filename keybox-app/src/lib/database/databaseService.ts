...
export class EncryptedDatabaseService {
  ...
  async initialize(
    ...
    if (masterKey) {
      await this.dataProtectionService.initializeProtectionKey(
        "user_auth",
        masterKey
      );
      await this.dataProtectionService.initializeProtectionKey(
  ...
  // User operations
  async createUser(
    user: Omit<DatabaseUser, "id" | "createdAt" | "updatedAt">
    ...
    const protectedUser = await DatabaseProtectionUtils.protectUserData(
      userWithTimestamps
    );
  ...
  async getUserByEmail(email: string): Promise<DatabaseUser | null> {
    ...
    return (await DatabaseProtectionUtils.unprotectUserData(
      protectedUser
    )) as DatabaseUser;
  }
  ...
  async updateUser(
    ...
    const protectedUpdates = await DatabaseProtectionUtils.protectUserData(
  ...
  async createCipher(
    cipher: Omit<
      DatabaseCipher,
      "id" | "createdAt" | "updatedAt" | "revisionDate"
    >
  ): Promise<DatabaseCipher> {
    this.ensureInitialized();
    ...
    // Protect sensitive fields
    const protectedCipher = await DatabaseProtectionUtils.protectCipherData(
      cipherWithTimestamps
    );
  ...
  async getCiphersByUserId(
    ...
    for (const protectedCipher of results) {
      const unprotectedCipher =
        await DatabaseProtectionUtils.unprotectCipherData(protectedCipher);
      unprotectedCiphers.push(unprotectedCipher as DatabaseCipher);
    }
  ...
  async updateCipher(
    cipherId: string,
    updates: Partial<DatabaseCipher>
  ): Promise<void> {
    this.ensureInitialized();
    ...
    // Protect sensitive fields in updates
    const protectedUpdates = await DatabaseProtectionUtils.protectCipherData(
      updatesWithTimestamp
    );
  ...
  // Folder operations
  async createFolder(
    folder: Omit<DatabaseFolder, "id" | "createdAt" | "updatedAt">
    ...
    // Protect folder name
    const protectedName = await this.dataProtectionService.protect(
      folder.name,
      "folder_data"
    );
  ...
  async getFoldersByUserId(userId: string): Promise<DatabaseFolder[]> {
    ...
    for (const folder of results) {
      const unprotectedName = await this.dataProtectionService.unprotect(
        folder.name,
        "folder_data"
      );
  ...
  async logSecurityEvent(
    event: Omit<DatabaseSecurityEvent, "id" | "timestamp">
  ): Promise<void> {
    this.ensureInitialized();
  ...
  async getSecurityEvents(
    ...
    return await this.connection!.query<DatabaseSecurityEvent>(sql, [
      userId,
      limit,
    ]);
  }
  ...
  // Backup operations
  async createBackup(
    backup: Omit<DatabaseBackup, "id" | "createdAt" | "updatedAt">
  ...
  async getBackupsByUserId(userId: string): Promise<DatabaseBackup[]> {
    ...
    const sql =
      "SELECT * FROM backups WHERE userId = ? ORDER BY createdAt DESC";
  ...
  async getStats(): Promise<{
    ...
    const [users, ciphers, folders, securityEvents, backups] =
      await Promise.all([
        this.connection!.query("SELECT COUNT(*) as count FROM users"),
        this.connection!.query(
          "SELECT COUNT(*) as count FROM ciphers WHERE deletedAt IS NULL"
        ),
